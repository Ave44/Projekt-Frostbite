...
class GoblinFangTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.GoblinFang = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.goblinFang = Surface((0, 0))

        self.item = GoblinFang(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = GoblinFang(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Goblin Fang")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.goblinFang)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.goblinFang)
