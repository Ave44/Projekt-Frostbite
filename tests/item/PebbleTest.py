...
class PebbleTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.Pebble = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.pebble = Surface((0, 0))

        self.item = Pebble(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = Pebble(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Pebble")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.pebble)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.pebble)
