...
class SmallMeatTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.SmallMeat = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.smallMeat = Surface((0, 0))

        self.item = SmallMeat(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = SmallMeat(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Small Meat")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.smallMeat)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.smallMeat)
