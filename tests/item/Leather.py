...
class LeatherTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.Leather = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.leather = Surface((0, 0))

        self.item = Leather(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = Leather(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.leather)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.leather)
