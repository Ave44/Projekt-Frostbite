...
class StoneAxeTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.StoneAxe = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.stoneAxe = Surface((0, 0))

        self.item = StoneAxe(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = StoneAxe(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Stone Axe")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.stoneAxe)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.stoneAxe)
