...
class Inventory(pygame.sprite.Sprite):
    def __init__(self,
        ...
        self._offset = (WINDOW_WIDTH // 2, WINDOW_HEIGTH // 2)
    ...
    def update(self) -> None:
        ...
        calculatedPos = (pos[0] - self._offset[0] + self._playerPos[0], pos[1] - self._offset[1] + self._playerPos[1])
        hoveredSlot = next(filter(lambda slot: (slot.rect.collidepoint(calculatedPos)), self._inventoryList), None)

        if hoveredSlot is None:
            return self.__handleNotHoveredState(pos, pressedMouseKeys)
