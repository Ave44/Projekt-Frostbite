...
class SharpRockTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.SharpRock = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.sharpRock = Surface((0, 0))

        self.item = SharpRock(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = SharpRock(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Sharp Rock")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.sharpRock)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.sharpRock)
