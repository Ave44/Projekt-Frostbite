...
class WoodenArmorTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.WoodenArmor = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.woodenArmor = Surface((0, 0))

        self.item = WoodenArmor(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = WoodenArmor(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Wooden Armor")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.woodenArmor)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.woodenArmor)
