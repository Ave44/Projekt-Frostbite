...
class BigMeatTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.BigMeat = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.bigMeat = Surface((0, 0))

        self.item = BigMeat(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = BigMeat(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Big Meat")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.bigMeat)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.bigMeat)
