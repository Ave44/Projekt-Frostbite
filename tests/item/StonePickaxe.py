...
class StonePickaxeTest(unittest.TestCase):
    def setUp(self) -> None:
        self.visibleSprites = Group()
        self.visibleSprites.items = Group()
        self.visibleSprites.savefileGroups = Group()
        self.visibleSprites.savefileGroups.StonePickaxe = Group()
        self.loadedImages = LoadedImages
        self.loadedImages.stonePickaxe = Surface((0, 0))

        self.item = StonePickaxe(self.visibleSprites, Vector2(0, 0), self.loadedImages)
        self.item2 = StonePickaxe(self.visibleSprites, Vector2(0, 0), self.loadedImages)
    ...
    def test_item_name_initializes(self):
        self.assertEqual(self.item.name, "Stone Pickaxe")

    def test_item_icon_initializes(self):
        self.assertEqual(self.item.icon, self.loadedImages.stonePickaxe)

    def test_item_img_initializes(self):
        self.assertEqual(self.item.image, self.loadedImages.stonePickaxe)
